...
Week currentWeek = Week(DateTime.now(), DateTime.now());
...
class _homeState extends State<home> {
  ...
  void calculateWeekDays() {
    ...
    currentWeek.firstDay = firstDayOfWeek;
  currentWeek.lastDay = lastDayOfWeek;

  int daylast = currentWeek.lastDay.day;
  int monthlast = currentWeek.lastDay.month;
  int yearlast = currentWeek.lastDay.year;
  int dayfirst = currentWeek.firstDay.day;
  int monthfirst = currentWeek.firstDay.month;
  int yearfirst = currentWeek.firstDay.year;
  ...
  Widget build(BuildContext context) {
    TimerBuilder.periodic(Duration(days: 1), builder: (context) {
      DateTime now = DateTime.now();
      DateTime firstDayOfWeeknew = now.subtract(Duration(days: now.weekday - 1));
