...
class _HomeState extends State<Home> {
  ...
  var todoBoxDone = Hive.box('todoBoxDone');
  @override
  void initState() {
    Hive.initFlutter();
    // Mở box với tên 'todoBox'
  ...
  void onClickToDoItemDone(ToDo todo) {
    ...
    if (todo.isDone) {
      setState(() {
        ...
        todoBoxDone.add(todo);
        var toDoToDelete =
            todoBox.values.firstWhere((toDo) => todo.isDone == true);
        todoBox.delete(toDoToDelete);
        todoList.removeWhere((todo) => todo.isDone == true);
        foundToDo = todoBox.values.toList().cast<ToDo>();
        toDoListDone=todoBoxDone.values.toList().cast<ToDo>();
      });
    } else {
      setState(() {
        todo.isDone = false;
        ...
        var toDoToDelete =
            todoBoxDone.values.firstWhere((toDo) => todo.isDone == false);
        todoBoxDone.delete(toDoToDelete);
        foundToDo = todoBox.values.toList().cast<ToDo>();
        toDoListDone=todoBoxDone.values.toList().cast<ToDo>();
      });
    }
  ...
  void onClickTodoItem(ToDo todo) {
    ...
    if (todo.isDone) {
      ...
      setState(() {
        ...
        var toDoToDelete =
            todoBox.values.firstWhere((todoo) => todoo.id == todo.id);
        todoBox.delete(toDoToDelete);
         foundToDo = todoBox.values.toList().cast<ToDo>();
        toDoListDone=todoBoxDone.values.toList().cast<ToDo>();
      });
  ...
  void onClickDeleteIcon(String id) {
    setState(() {
      ...
      toDoListDone.removeWhere((todoo) => todoo.id == id);
       var toDoToDeletedone = todoBoxDone.values.firstWhere((todoo) => todoo.id == id);
      todoBoxDone.delete(toDoToDeletedone);

      var toDoToDelete = todoBox.values.firstWhere((todoo) => todoo.id == id);
      todoBox.delete(toDoToDelete);
       foundToDo = todoBox.values.toList().cast<ToDo>();
        toDoListDone=todoBoxDone.values.toList().cast<ToDo>();
    });
