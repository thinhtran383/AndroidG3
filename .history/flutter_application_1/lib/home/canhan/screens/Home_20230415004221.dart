...
class _HomeState extends State<Home> {
  ...
  void onClickToDoItemDone(todo todo) {
    ...
    if (todo.isDone == true) {
      var todoToDeletedone =
          tasksBoxDone.values.firstWhere((todo1) => todo1.id == todo.id);
      ...
      var todoToDeletedone1 =
          tasksBoxDone.values.firstWhere((todo) => todo.id == todo.id);
      if (todoToDeletedone1 != null) {
        tasksBoxDone.delete(todoToDeletedone1.key);
      }
      setState(() {
        todo.isDone = false;
        // todoList.add(todo);
        tasksBox.put(todo.id,todo);
        foundToDo = tasksBox.values.toList();
  ...
  void onClickTodoItem(ToDo todo) {
    ...
    var todoToDeletedone =
        tasksBox.values.firstWhere((todo) => todo.id == todo.id);
    if (todoToDeletedone != null) {
      tasksBox.delete(todoToDeletedone.key);
    }
    if (todo.isDone == true) {
  ...
  void onClickDeleteIcon(String id) {
    ...
    if (todoToDelete != null) {
     ...
     var todoToDelete1 = tasksBoxDone.values.firstWhere((todo) => todo.id == id);
    if (todoToDelete1 != null) {
      tasksBoxDone.delete(todoToDelete1.key);
