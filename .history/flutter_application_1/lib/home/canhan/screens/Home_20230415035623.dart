...
class _HomeState extends State<Home> {
  ...
  void onClickToDoItemDone(todo todo1) {
    ...
    if (todo1.isDone == true) {
      setState(() {
        toDoListDone.removeWhere((todo2) => todo2.id == todo1.id);
      foundToDo.add(toDoListDone.where((todo2) => todo2.id == todo1.id).toList());
             List<todo> todosToRemove = toDoListDone.where((todo2) => todo2.id == todo1.id).toList();
      toDoListDone.removeWhere((todo2) => todo2.id == todo1.id);

      // Thêm các phần tử đã xóa vào danh sách foundToDo
      foundToDo.add(todosToRemove);
